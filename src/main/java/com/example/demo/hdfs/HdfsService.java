...
public class HdfsService {
    ...
    private String path = "hdfs://172.18.1.51:8020,hdfs://172.18.1.52:8020,hdfs://172.18.1.53:8020,hdfs://172.18.1.54:8020";
    private String username = "hdfs";

    private static String hdfsPath = "hdfs://172.18.1.51:8020";
    private static String hdfsName = "hdfs";
     ...
     * @throws Exception
     */
    public static void createFile(String path, byte[] fileByte, String fileName) throws Exception {
        if (StringUtils.isEmpty(path) || null == fileByte) {
            return;
        }
        FileSystem fs = getFileSystem();
        // 上传时默认当前目录，后面自动拼接文件的目录
    ...
    public static List<String> listFile2(String path) throws Exception {
        if (StringUtils.isEmpty(path)) {
            return null;
        }
        if (!existFile(path)) {
        ...
        while (filesList.hasNext()) {
            LocatedFileStatus next = filesList.next();
            Path filePath = next.getPath();
            returnList.add(filePath.toString());
        }
        fs.close();
        return returnList;
    }
     ...
     * @throws Exception
     */
    public static void downloadFiles(List<String> path, String downloadPath) throws Exception {
        ...
        for (int i = 0; i < path.size(); i++) {
            // 上传路径
            Path clientPath = new Path(path.get(i));
            // 调用文件系统的文件复制方法，第一个参数是否删除原文件true为删除，默认为false
            fs.copyToLocalFile(false, clientPath, serverPath);
        }
